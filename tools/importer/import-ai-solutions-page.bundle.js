...
var import_ai_solutions_page_default = {
  transform: (payload) => {
    ...
    pageBlocks.forEach((block) => {
      if (block.name === "hero") {
        const video = block.element.querySelector("video");
        if (video) {
          const source = video.querySelector('source[type="video/mp4"]') || video.querySelector("source");
          const mp4Src = source?.getAttribute("src") || video.getAttribute("src");
          if (mp4Src) {
            heroVideoData.push({
              url: mp4Src.startsWith("//") ? `https:${mp4Src}` : mp4Src,
              parent: block.element.parentElement
            });
          }
        }
      }
    ...
    heroVideoData.forEach(({ url: videoUrl, parent }) => {
      if (!parent) return;
      const heroTable = parent.querySelector("table");
      if (!heroTable) return;
      const header = heroTable.querySelector("tr:first-child th, tr:first-child td");
      if (!header || !header.textContent.trim().toLowerCase().startsWith("hero")) return;
      const bgRow = heroTable.querySelector("tr:nth-child(2)");
      if (!bgRow) return;
      const img = bgRow.querySelector("img");
      if (img) {
        img.title = `data-video:${videoUrl}`;
      }
    });
...
export {
  import_ai_solutions_page_default as default
};
